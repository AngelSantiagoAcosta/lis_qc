    ...
    for sampleID in unique_sampleIDs:
        ...
        if check_cbc:
            ...
            if cbc_bool:
                ...
                cbc_filter = (filtered_df["Test ASTM/HL7/SDF 1 Code"].isin(cbc_list))
            ...
            if chem_bool or tsh_bool :
                df.loc[sample_filter,"Validation 28"] ="passed"
                chem_filter = (filtered_df["Test ASTM/HL7/SDF 1 Code"].isin(chem_list))
    ...
    for sampleID in unique_samples:
        ...
        for tube in tubeID_list:
            ...
            if type(panel) == NoneType: # since we maybe have blank tube, or tube == "Slide" we want to skip those
                continue
            tube_filter = (sample_df["Tube/Container Type"] == tube) & (~sample_df["Test ASTM/HL7/SDF 1 Code"].isnull())
            found_methods = sample_df.loc[tube_filter]["Test ASTM/HL7/SDF 1 Code"].to_list()
...
def apply_validation_checks(df):
    ...
    df["Validation 22"] = np.where((~is_number(df["Reported As"].astype(str))) & (df["Reported As"].str.lower().isin(acceptable_string_list_22) == False) & ((df["Reported As"].str.lower().isin(acceptable_string_list_22) == False) & (df["Reported As"].str.startswith(("<",">"),na=False)==False)),"Unacceptable String Value in Reported As","passed")
...
def read_excel_file(input):
    ...
    if is_csv:
        df = pd.read_csv(input, encoding='cp1252', keep_default_na=False, na_values="")
...
"Specimen hemolyzed","Test not performed due to special occurrence","Quantity not sufficient for testing",
...
if __name__ == '__main__':   
    ...
    dir = r"C:\Users\AngelAcosta\Documents\Python Scripting\QC Validation\LIS data"
    process_all(dir)
